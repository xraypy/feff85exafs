...
#             yield check_stat, p
#     else:
#         assert True, "no fit tests for UO2"


# def check_param(param, part):
#     bl=getattr(getattr(fut.blfit.params, param), part)
#     tr=getattr(getattr(fut.trfit.params, param), part)
#     assert abs(bl-tr) < fut.epsilon, "%s of fitting parameter %s evaluated inconsitently for UO2" % (part, param)

# def check_stat(param):
#     bl=getattr(fut.blfit.params, param)
#     tr=getattr(fut.trfit.params, param)
#     assert abs(bl-tr) < fut.epsilon, "statistic %s evaluated inconsitently for UO2" % param
