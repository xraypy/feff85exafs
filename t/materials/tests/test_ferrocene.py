...
def check_columns(part):
    ...
    assert this, "comparison of %s for path 1 in ferrocene" % part
...
def check_feff(index, part):
    ...
    assert this, "comparison of %s for path %d in ferrocene" % (part, index)


################################################################################

def test_terms():
    assert fut.feffterms(), "some feff terms calculated incorrectly for ferrocene"

def test_s02():
    assert fut.s02() == fut.s02('baseline'), "s02 calculated incorrectly for ferrocene"
...
def test_fit():
    if not fut.feffran: fut.run()
    if skip_data_test:
        yield check_null, "skipping data test for ferrocene"
        ...
        yield check_null, "no data tests for ferrocene"
...
def check_param(param, part):
    ...
    assert abs(bl-tr) < fut.epsilon, "%s of fitting parameter %s evaluated inconsistently for ferrocene" % (part, param)

def check_stat(param):
    bl=getattr(fut.blfit.params, param)
    tr=getattr(fut.trfit.params, param)
    assert abs(bl-tr) < fut.epsilon, "statistic %s evaluated inconsistently for ferrocene" % param

################################################################################

def test_clean():
    fut.clean()
    assert not isdir(fut.testrun), "successfully cleaned up ferrocene calculation"
