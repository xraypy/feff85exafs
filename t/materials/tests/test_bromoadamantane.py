...
def test_terms():
    assert fut.feffterms(), "some feff terms calculated incorrectly for bromoadamantane"

def test_s02():
    assert fut.s02() == fut.s02('baseline'), "s02 calculated incorrectly for bromoadamantane"
...
def test_fit():
    if not fut.feffran: fut.run()
    if skip_data_test:
        yield check_null, "skipping data test for bromoadamantane"
        ...
        yield check_null, "no data tests for bromoadamantane"
...
def check_param(param, part):
    ...
    assert abs(bl-tr) < fut.epsilon, "%s of fitting parameter %s evaluated inconsistently for bromoadamantane" % (part, param)

def check_stat(param):
    bl=getattr(fut.blfit.params, param)
    tr=getattr(fut.trfit.params, param)
    assert abs(bl-tr) < fut.epsilon, "statistic %s evaluated inconsistently for bromoadamantane" % param
